...
public class MostCommonWord819 {
	public String mostCommonWord(String paragraph, String[] banned) {
		...
		for (String str : paragraph.replaceAll("^[a-zA-Z0-9]", "").toLowerCase().split(" ")) {
			if (!bannedWordSet.contains(str) && !str.equals("")) {
		...
		for (String key : map.keySet()) {
			if (resultString == "" || map.get(key) > map.get(resultString)) {
				resultString = key;
			}
